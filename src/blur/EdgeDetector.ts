...
import { EdgeCrossingType } from '../model/EdgeCrossingType';
import { EdgeCrossing } from '../model/EdgeCrossing';
...
export class EdgeDetector {
        ...
        for(var i=0; i<points.length; i++){
            ...
            if(p1.x > this.imageWidth - sideDistance && p2.x < sideDistance){ // left to right
                if(p1.y > this.imageHeight/2){ // bottom
                    ...
                    edgeCrossings.push({
                        type: EdgeCrossingType.BOTTOM_LEFT_RIGHT,
                        index: i
                    });
                } else { // top
                    //console.log('TOP_LEFT_RIGHT');
                    edgeCrossings.push({
                        type: EdgeCrossingType.TOP_LEFT_RIGHT,
                        index: i
                    });
                }
                i+=2;
            ...
            if(p1.x < sideDistance && p2.x > this.imageWidth - sideDistance){ // right to left
                if(p1.y > this.imageHeight/2){ // bottom
                    ...
                    edgeCrossings.push({
                        type: EdgeCrossingType.BOTTOM_RIGHT_LEFT,
                        index: i
                    });
                } else { // top
                    //console.log('TOP_RIGHT_LEFT');
                    edgeCrossings.push({
                        type: EdgeCrossingType.TOP_RIGHT_LEFT,
                        index: i
                    });
                }
                i+=2;
    ...
    private static extendSingleCrossing(points: Point[], edgeCrossing: EdgeCrossing): Point[][]{
        ...
        switch (edgeCrossing.type) {
            case EdgeCrossingType.TOP_LEFT_RIGHT:
                point1 = {x: this.imageWidth, y: 0};
                point2 = {x: 0, y: 0};
                break;
            
            case EdgeCrossingType.TOP_RIGHT_LEFT:
                point1 = {x: 0, y: 0};
                point2 = {x: this.imageWidth, y: 0};
                break;
            
            case EdgeCrossingType.BOTTOM_LEFT_RIGHT:
                point1 = {x: this.imageWidth, y: this.imageHeight};
                point2 = {x: 0, y: this.imageHeight};
                break;

            case EdgeCrossingType.BOTTOM_RIGHT_LEFT:
    ...
    private static extendDoubleCrossing(points: Point[], edgeCrossing1: EdgeCrossing, edgeCrossing2: EdgeCrossing): Point[][]{
        ...
        if(edgeCrossing2.type === EdgeCrossingType.BOTTOM_LEFT_RIGHT || edgeCrossing2.type === EdgeCrossingType.TOP_LEFT_RIGHT) leftRight = edgeCrossing2;
        if(edgeCrossing1.type === EdgeCrossingType.BOTTOM_RIGHT_LEFT || edgeCrossing1.type === EdgeCrossingType.TOP_RIGHT_LEFT) rightLeft = edgeCrossing1;
        if(edgeCrossing2.type === EdgeCrossingType.BOTTOM_RIGHT_LEFT || edgeCrossing2.type === EdgeCrossingType.TOP_RIGHT_LEFT) rightLeft = edgeCrossing2;

        if(leftRight === undefined) throw new Error('Left to right edge crossing is missing.');
        if(rightLeft === undefined) throw new Error('Right to left edge crossing is missing.');
    ...
    private static extendWithEdge(points: Point[], end: Point) {        
        ...
        if(!(last.x === 0 || last.x === this.imageWidth)) throw new Error('Points to be connected are not edge points.');
        if(Math.abs(end.y - last.y) <= 1) return; // there is no gap between them
